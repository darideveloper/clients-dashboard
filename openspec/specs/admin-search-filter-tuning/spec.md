# admin-search-filter-tuning Specification

## Purpose
Cross-cutting search/filter conventions for the ourlives admin: exact per-admin `search_fields` with index-friendly prefixes, `search_help_text` on heavy admins, and filter-over-search rules. Created by archiving change admin-search-filter-tuning.
## Requirements
