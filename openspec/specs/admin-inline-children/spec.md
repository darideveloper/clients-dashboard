...
## Purpose
Inline editing of Contact, OrganizationAddress, and OrderItem via their parent Organization/Order change forms, coexisting with their standalone changelists (standalone registration is required for Order contact autocomplete filters).
## Requirements
### Requirement: Contact inline on Organization

The system SHALL provide `ContactInline(unfold.admin.StackedInline)` in `ourlives/admin.py` with `model=Contact`, `extra=0`, `autocomplete_fields=("contact_type",)`, full add/change/delete through the parent, alongside the standalone `ContactAdmin` registration (see `crm-admin-registration`). (No `tab` attribute: Unfold 0.77 auto-renders inlines as change-form tabs.) `OrganizationAdmin` SHALL include it in `inlines`.
...
#### Scenario: Standalone Contact changelist kept

- **WHEN** any user requests `/admin/ourlives/contact/` or `/admin/ourlives/contact/add/`
- **THEN** the registered `ContactAdmin` changelist/add form renders (model registered; registration backs the Order contact autocomplete filters)

### Requirement: OrganizationAddress inline on Organization

The system SHALL provide `OrganizationAddressInline(unfold.admin.StackedInline)` in `ourlives/admin.py` with `model=OrganizationAddress`, `extra=0`, `autocomplete_fields=("country",)`, full add/change/delete through the parent, alongside the standalone `OrganizationAddressAdmin` registration (see `crm-admin-registration`). (No `tab` attribute: Unfold 0.77 auto-renders inlines as change-form tabs.) `OrganizationAdmin` SHALL include it in `inlines` alongside `ContactInline`.
...
#### Scenario: Standalone address changelist kept

- **WHEN** any user requests `/admin/ourlives/organizationaddress/`
- **THEN** the registered `OrganizationAddressAdmin` changelist renders (model registered)

### Requirement: OrderItem Unfold inline alongside standalone admin

The system SHALL provide `OrderItemInline(unfold.admin.TabularInline)` (migrated from `django.contrib.admin.TabularInline`) with `model=OrderItem`, `extra=0`, `autocomplete_fields=("product",)`, readonly `line_total_display` (`@admin.display` over model `line_total`), full add/change/delete through the parent, alongside the standalone `OrderItemAdmin` registration (see `crm-admin-registration`). `OrderAdmin.inlines` SHALL equal `(OrderItemInline,)`.
...
#### Scenario: Standalone OrderItem changelist kept

- **WHEN** any user requests `/admin/ourlives/orderitem/`
- **THEN** the registered `OrderItemAdmin` changelist renders (model registered)
