# form-webhook-ingestion Specification

## Purpose
The dedicated Ourlens webhook POST /webhooks/ourlens/ that validates a shared-secret body.token, maps an n8n mapping into CRM rows (Order, OrderItem(s), InvitationCode(s), audit trail), processes both production and test/dev submissions, and records a FormWebhookEvent audit row.
## Requirements
