...
The system SHALL provide a `Currency` model with `code` (CharField max_length=3, unique), `name` (CharField max_length=100), `symbol_left` (CharField max_length=10, blank=True), `symbol_right` (CharField max_length=10, blank=True), `exchange_rate` (DecimalField max_digits=10, decimal_places=2), `country` (ForeignKey to `Country`, null=True, blank=True, on_delete=SET_NULL, related_name="currencies"), `active` (BooleanField default True), `__str__` returning `code`, and `Meta` ordering by `code` with `verbose_name`/`verbose_name_plural` matching existing model style.

#### Scenario: Create currency with country

- **WHEN** a Currency is created with code="USD", name="US Dollar", symbol_left="$", exchange_rate="1.00", country=<USA>
- **THEN** it persists with `active` True and `str()` returns "USD"

#### Scenario: Country is nullable

- **WHEN** a Currency is created with code="EUR", name="Euro" and no country
- **THEN** it persists with `country` None

#### Scenario: Country delete sets null

- **WHEN** the linked Country is deleted
- **THEN** the Currency survives with `country` None (SET_NULL, no ProtectedError)
