...
- `agreed_scans_total`: per-currency mapping of `SUM(number_of_scans × cost_per_scan)` over related orders, with null `number_of_scans`/`cost_per_scan` treated as 0 (same rule as `Order.total_agreed_price`). Each order amount is attributed to the order's `currency` when set, otherwise to its `pilot_currency`, otherwise to a `No Currency` bucket. Pilot orders are included.
- `catalog_items_total`: per-currency mapping of `SUM(quantity × unit_price)` over related orders' items. Each item amount is attributed to the parent order's `currency` when set, otherwise its `pilot_currency`, otherwise the line's `Product.currency`; amounts with no resolvable currency go under a `No Currency` bucket.
...
- **THEN** its agreed amount is attributed to `No Currency`
...
- **THEN** its line total is attributed to the line's `Product.currency`, or to `No Currency` when that is also unresolvable
...
- `combined_total` / "Total Order Value" / "Agreed scans total + catalog items total, per currency. Amounts in different currencies are never added together."
