...
## Purpose
Define where and how the django-unfold-styled `User` and `Group` admins are registered, the forms they use, and the base class they inherit from, so the admin UI for authentication models matches the rest of the project.
