erDiagram
    %% Source: ourlives/models.py is source of truth (migrations 0009-0012 applied). 16 models implemented.
    ...
    organizations {
        ...
        int assigned_rep_id FK "e.g. 7->John nullable SET_NULL, Organization.assigned_rep"
    }
    ...
    invitation_codes {
        ...
        int order_id FK "e.g. 82->OL-82 nullable PROTECT, FK->orders"
        int code_type_id FK "e.g. 1->Up to 5 nullable PROTECT, FK->code_types"
    ...
    countries {
        ...
        string region "e.g. NA, EU nullable blank=True"
    ...
    currencies {
        ...
        string symbol_left "e.g. $ nullable blank=True"
        string symbol_right "e.g. empty nullable blank=True"
        decimal exchange_rate "e.g. 1.00, 1.35 CAD required"
        boolean active "e.g. true required"
    }

    products {
        int product_id PK "e.g. 1 distinct from projects, field_q8zvr2"
        int currency_id FK "e.g. 1->USD required PROTECT"
        string name "e.g. Micro Pilot required, field_q8zvr2"
        string tier "e.g. micro nullable blank=True"
        decimal unit_price "e.g. 995.00 required, data-frmprice"
        boolean active "e.g. true required"
        string description "e.g. Pilot bundle 500 tokens nullable"
    }

    code_types {
        int code_type_id PK "e.g. 1"
        string code UK "e.g. up_to_5 required"
        string name UK "e.g. Up to 5 Additional Codes required, field_uwfjk2"
        int max_codes "e.g. 5 required independent of invitation_codes.max_use"
        boolean active "e.g. true required"
        string description "e.g. Small team bundle, nullable"
    }
    ...
    organization_addresses {
        ...
        int organization_id FK "e.g. 12->Acme required CASCADE, FK->organizations"
    ...
    contact_types {
        ...
        string name UK "Primary Contact, Invoice Contact, Billing"
        boolean active "true = selectable in form"
        string description "Extensible contact role, from 616/619 nullable"
    }

    contacts {
        int contact_id PK "e.g. 31"
        int organization_id FK "e.g. 12 -> Acme Ltd, required CASCADE, ex-company_id"
        int contact_type_id FK "FK->contact_types PROTECT, e.g. 1=Primary"
        ...
        string phone "e.g. +44 7700 900123 tel, nullable blank=True"
    }

    order_types {
        int order_type_id PK "e.g. 1=Pilot, 2=Standard, 3=Trial"
        string code UK "pilot, standard, trial, renewal"
        string name UK "Pilot Order, Standard Order, Trial Order"
        boolean active "true = show as radio option in frmrules"
        string description "Extensible order type, nullable"
    }

    orders {
        int order_id PK "e.g. 82"
        string order_number UK "OL-82 auto default=generate_order_number, from field_be8ml"
        int organization_id FK "e.g. 12 Acme Ltd, required PROTECT, ex-company_id"
        int rep_id FK "e.g. 7 John Doe, required PROTECT"
        int primary_contact_id FK "nullable PROTECT, e.g. 31 Alice Smith"
        int invoice_contact_id FK "nullable PROTECT, e.g. 32 Bob Smith"
        int currency_id FK "nullable SET_NULL, for non-pilot e.g. 1=USD, field_mo8wu"
        int pilot_currency_id FK "nullable SET_NULL, for pilot e.g. 3=GBP field_5znvs"
        boolean is_pilot_order "e.g. true->Pilot @property from M2M code=pilot, no column"
        boolean is_upgrade_from_pilot "e.g. false default=False, field_r9jxe2"
        boolean is_referral_order "e.g. false default=False, field_53psq2"
        ...
        string form_entry_key "e.g. kR3x9 Formidable key nullable"
        datetime submitted_at "e.g. 2026-09-02T01:45:00Z auto_now_add"
        boolean hcaptcha_verified "e.g. false default=False"
    }

    order_items {
        int order_item_id PK "e.g. 201"
        int order_id FK "e.g. 82->OL-82 required CASCADE"
        int product_id FK "e.g. 2->Regional Pilot required PROTECT"
    ...
    %% Relationships — mirrors models.py on_delete / related_name
    countries ||--o{ organization_addresses : "has"
    countries }o--o{ currencies : "has M2M Currency.countries blank=True"
    currencies ||--o{ products : "has PROTECT"
    currencies ||--o{ orders : "currency_for_non_pilot SET_NULL"
    currencies ||--o{ orders : "pilot_currency SET_NULL"

    organizations ||--o{ organization_addresses : "has CASCADE"
    organizations ||--o{ contacts : "has CASCADE"
    organizations ||--o{ orders : "places PROTECT"
    organizations ||--o{ invitation_codes : "owns PROTECT"

    projects ||--o{ invitation_codes : "defines PROTECT"

    reps ||--o{ organizations : "assigned_to SET_NULL"
    reps ||--o{ orders : "owns PROTECT"

    contact_types ||--o{ contacts : "categorizes PROTECT"
    contacts ||--o{ orders : "primary_for PROTECT nullable"
    contacts ||--o{ orders : "invoice_for PROTECT nullable"

    orders }o--o{ order_types : "has implicit M2M Order.order_types blank=True, no notes column"
    orders ||--o{ order_items : "contains CASCADE"
    products ||--o{ order_items : "ordered_as PROTECT"

    orders ||--o{ invitation_codes : "has PROTECT nullable"
    code_types ||--o{ invitation_codes : "categorizes PROTECT nullable"
